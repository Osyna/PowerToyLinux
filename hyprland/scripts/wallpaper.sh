...
# Store original wallpaper before effects
original_wallpaper=$wallpaper
...
echo ":: Execute wallust with original wallpaper (for colors)"
if ! "$HOME/dotfiles/.config/wallust/run-wallust.sh" "$original_wallpaper"; then
    echo ":: Wallust color generation failed"
    exit 1
fi
